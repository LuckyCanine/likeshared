...
def index():
	...
	friendslikes = fql(
	   "select page_id, uid from page_fan where uid in ( select uid2 from friend where uid1 = me() limit 5) and page_id in (select page_id from page_fan where uid = me())", access_token)
